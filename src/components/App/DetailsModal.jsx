...
export default function DetailsModal({ open, onClose, data, loading }) {
    ...
    return (
        <div className="modal d-block" tabIndex="-1" role="dialog" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={handleBackdrop}>
                                ...
                                <div className="col-12 col-md-8">
                                    ...
                                    <div className="row g-2">
                                        ...
                                        {Website && (
                                            <div className="col-12">
                                                <strong>Website:</strong> <a href={Website} target="_blank" rel="noreferrer noopener">{Website}</a>
                                            </div>
                                        )}
                                    </div>
                                    {Array.isArray(Ratings) && Ratings.length > 0 && (
                                        <ul className="list-group list-group-flush mt-3">
                                            {Ratings.map((r) => (
                                                <li className="list-group-item" key={r.Source}>{r.Source}: {r.Value}</li>
                                            ))}
                                        </ul>
                                    )}
                                </div>
